...
// Check if CBA and required functions are available, if not, delay initialization
if (isNil "CBA_missionTime" || isNil "GRAD_simpleVehicleRespawn_fnc_check" || CBA_missionTime < 3) exitWith {
    diag_log format ["grad-vehicleRespawn: CBA not ready yet, delaying initialization for vehicle %1", _vehObj];
    [{
        _this call GRAD_simpleVehicleRespawn_fnc_add;
    }, [_vehObj, _respawnWhenNotDead, _loopInterval], 2] call CBA_fnc_waitAndExecute;
};
